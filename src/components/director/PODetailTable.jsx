...
const PODetailTable = () => {
  ...
  // Get filterFromCard from location.state, default to 'all' if not provided
  const filterFromCard = location.state?.filter || "all";

  // State for filters from FilterBar
  ...
  // Update filter state when the FilterBar changes
  ...
  const getFilteredData = () => {
    ...
    // Apply filters passed from the stat card (PO Age)
    ...
    // Apply additional filters (from FilterBar or passed from the previous screen)
    ...
    // Optional: Add fromDate/toDate logic here based on invoiceDate if required
    // Apply PO Age filter if user selects it here
  ...
  useEffect(() => {
    // To ensure the PO Age filter is visible when coming to this page
    setFilters((prevState) => ({
      ...prevState,
      poAge: userFilters.poAge || prevState.poAge, // Ensure that PO Age is updated on mount if needed
    }));
  }, [userFilters]);
  ...
  return (
    <div className="p-6 bg-slate-800 rounded-xl text-white">
      ...
      {/* Render FilterBar with filter state and enable PO Age filtering */}
